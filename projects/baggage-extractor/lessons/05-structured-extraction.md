...
上一课：[领域 Schema](./04-domain-schema.md) · [课程索引](./README.md) · 下一课：[固定评估与评分](./06-fixed-evaluation.md)
...
可选择其中一个小测试做[AI 编程协作练习](../../../docs/assessment.md#项目交付与-ai-编程协作)，但自己审查 diff 和结果。
...
在 `projects/baggage-extractor` 项目目录使用已有虚拟环境，安装方式见[环境指南](../../../docs/setup.md)。
...
- [ ] 如实记录自己的离线验证与真实模型验证边界，不把历史报告当作本次运行证据。

下一课进入已发布的[第 6 课：固定评估与评分](./06-fixed-evaluation.md)，随后完成
[第 7 课：FastAPI 服务化](./07-fastapi-service.md)，不提前引入 Agent 框架。

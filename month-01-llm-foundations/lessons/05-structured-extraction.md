...
> **状态：规划中。** 本课已有实施任务和验收标准，但仓库中尚无结构化请求契约、
> 提取服务、独立提取 CLI 或对应参考测试。
...
以下命令需在实现完成后运行。当前规划状态下，新增测试尚不存在。
...
- [ ] 新 CLI 与测试不修改实验入口职责。
- [ ] 完整 pytest 与 Ruff 通过，常规测试没有网络调用。
- [ ] 真实冒烟测试是显式可选的，并记录验证边界。
- [ ] 更新 `docs/schema.md`、课程状态和作者进度；没有真实验证时不宣称完成。
